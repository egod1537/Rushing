...
namespace Map.MapGenerator
{
    ...
    {
        public MapTracker(int _n, int _m)
            : base(_n, _m)
        {
        }
        public MapTracker(int sx, int sy, MapLayout layout)
            : base(sx, sy, layout)
        {
        }
        public MapTracker(Vector2Int start, MapLayout layout)
            : base(start, layout)
        {
        }
        public MapTracker(int _n, int _m, List<string> _arr)
            : base(_n, _m, _arr)
        {
        }
        ...
        public string Track(List<int> ret)
        {
            ...
            dst.Resize(scc.n, scc.n, -1);
            path.Resize(scc.n, scc.n, "");
            ...
            List<List<int>> dst, List<List<string>> path)
        {
            ...
            foreach (int st in g)
            {
                ...
                dp.Resize(sz, 1<< starCnt, -1);
                prv.Resize(sz, 1<<starCnt, new Vector2Int(-1, -1));
        ...
        private string Routing(List<int> route, List<List<int>> dst, List<List<string>> path)
        {
            ...
            for (int i = 0; i < sz; i++) nid.Add(new Dictionary<int, int>());
            for (int i = 0; i < sz; i++)
            {
